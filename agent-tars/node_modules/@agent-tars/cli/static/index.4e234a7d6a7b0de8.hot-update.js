 ...
 */ const processEventAction = (0,jotai__WEBPACK_IMPORTED_MODULE_6__.atom)(null, (get1, set, params)=>{
    const { sessionId, event } = params;
    const replayState = get1(_atoms_replay__WEBPACK_IMPORTED_MODULE_5__.replayStateAtom);
    ...
    switch(event.type){
        ...
        case 'assistant_message':
            handleAssistantMessage(get1, set, sessionId, event);
            break;
        case 'assistant_streaming_message':
            if (!isReplayMode) {
                handleStreamingMessage(get1, set, sessionId, event);
            }
            break;
        case 'assistant_thinking_message':
        case 'assistant_streaming_thinking_message':
            handleThinkingMessage(get1, set, sessionId, event);
            break;
        case 'tool_call':
        ...
        case 'plan_finish':
            ...
            break;
        case 'final_answer':
            handleFinalAnswer(get1, set, sessionId, event);
            break;
        case 'final_answer_streaming':
            if (!isReplayMode) {
                handleFinalAnswerStreaming(get1, set, sessionId, event);
            }
            break;
    }
});
const updateProcessingStatusAction = (0,jotai__WEBPACK_IMPORTED_MODULE_6__.atom)(null, (get1, set, status)=>{
 ...
 */ function handleAssistantMessage(get1, set, sessionId, event) {
    // 获取消息ID
    ...
    // 检查是否需要关联最近的环境输入
    const currentMessages = get1(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom)[sessionId] || [];
    // 从后往前查找最近的环境输入
 ...
 */ function handleStreamingMessage(get1, set, sessionId, event) {
    set(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom, (prev)=>{
        const sessionMessages = prev[sessionId] || [];
 ...
 */ function handleThinkingMessage(get1, set, sessionId, event) {
    set(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom, (prev)=>{
        const sessionMessages = prev[sessionId] || [];
        ...
        if (imageContent && imageContent.image_url) {
            ...
            if (currentPanel && currentPanel.type === 'browser_vision_control') {
                set(_atoms_ui__WEBPACK_IMPORTED_MODULE_2__.activePanelContentAtom, {
                    ...currentPanel,
                    type: 'browser_vision_control',
                    title: `${currentPanel.title} / Screen Update`,
                    timestamp: event.timestamp,
 ...
 */ function handleFinalAnswer(get1, set, sessionId, event) {
...
function handleFinalAnswerStreaming(get1, set, sessionId, event) {
    const messageId = event.messageId || `final-answer-${(0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])()}`;
    // 从当前消息列表中查找已有的相同 messageId 的消息
    const messages = get1(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom)[sessionId] || [];
    ...
    const prevActivePanelContent = get1(_atoms_ui__WEBPACK_IMPORTED_MODULE_2__.activePanelContentAtom);
    if (!prevActivePanelContent || prevActivePanelContent.messageId !== messageId) {
        ...
        // 当流式生成完成时，更新消息的完整内容
        const fullContent = get1(_atoms_ui__WEBPACK_IMPORTED_MODULE_2__.activePanelContentAtom).source;
        set(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom, (prev)=>{
            const sessionMessages = prev[sessionId] || [];
...
(() => {
__webpack_require__.h = () => ("7e95699b0c596bd3")
})();

}
);
//# sourceMappingURL=index.4e234a7d6a7b0de8.hot-update.js.map