 ...
 */ function handleToolResult(set, sessionId, event) {
    // 获取之前存储的参数信息
    const args1 = toolCallArgumentsMap.get(event.toolCallId);
    // 添加调试日志来跟踪内容格式
    ...
    // 添加调试日志，显示确定的类型
    console.log(`Determined type for ${event.name}: ${result1.type}`);
    // 如果是browser_vision_control工具，检查是否有关联的环境输入面板
    if (result1.type === 'browser_vision_control') {
        // 获取当前活动面板内容
    ...
    _atoms_tool__WEBPACK_IMPORTED_MODULE_1__.toolCallResultMap.set(result1.toolCallId, result1);
    ...
    set(_atoms_message__WEBPACK_IMPORTED_MODULE_0__.messagesAtom, (prev)=>{
        ...
        const messageIndex = [
            ...
            var _m_toolCalls;
            return (_m_toolCalls = m.toolCalls) === null || _m_toolCalls === void 0 ? void 0 : _m_toolCalls.some((tc)=>tc.id === result1.toolCallId);
        });
        if (messageIndex !== -1) {
...
(() => {
__webpack_require__.h = () => ("e8367e7985c3de58")
})();

}
);
//# sourceMappingURL=index.360ae44a4380e2fb.hot-update.js.map