 ...
 */ function handleEnvironmentInput(set, sessionId, event) {
    ...
    if (Array.isArray(event.content)) {
        ...
        if (imageContent && imageContent.image_url) {
            set(_atoms_ui__WEBPACK_IMPORTED_MODULE_2__.activePanelContentAtom, (prev)=>{
                // 获取之前面板的增强控制信息（如果有）
                const prevControls = prev && prev.type === 'browser_vision_control' ? {
                ...
                const content = {
                    // 如果有之前的增强控制，保持相同类型
                    type: prevControls ? 'browser_vision_control' : 'image',
                    // 设置为新的图像内容
                    source: prevControls ? prevControls.source : event.content,
                    title: event.description || 'Browser Screenshot',
                    timestamp: event.timestamp,
                    environmentId: event.id,
                    // 保留之前的控制信息
                    ...prevControls && {
                        toolCallId: prevControls.toolCallId,
                        arguments: prevControls.arguments
                    },
                    // 始终用最新图像更新原始内容
...
(() => {
__webpack_require__.h = () => ("9fed73b135177ec8")
})();

}
);
//# sourceMappingURL=index.884c80698bb62b1a.hot-update.js.map