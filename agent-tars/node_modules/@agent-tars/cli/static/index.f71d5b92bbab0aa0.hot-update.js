 ...
 */ const ToolCalls = (param)=>{
    ...
    const getToolCallStatus = (toolCall)=>{
        ...
        if ((_toolCall_function = toolCall.function) === null || _toolCall_function === void 0 ? void 0 : _toolCall_function.arguments) {
            try {
        ...
        switch(status){
            case 'constructing':
                return /*#__PURE__*/ (0,react_jsx_dev_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxDEV)(framer_motion__WEBPACK_IMPORTED_MODULE_3__.motion.div, {
                    ...
                    children: /*#__PURE__*/ (0,react_jsx_dev_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxDEV)(react_icons_fi__WEBPACK_IMPORTED_MODULE_4__.FiEdit3, {
                        size: 16,
    ...
    const getToolDescription = (toolCall, status)=>{
        try {
            ...
            switch(toolCall.function.name){
                case 'web_search':
                    return args.query ? `"${args.query}"` : status === 'constructing' ? 'preparing query...' : '';
                case 'browser_navigate':
                    return args.url || (status === 'constructing' ? 'preparing navigation...' : '');
                case 'browser_vision_control':
                case 'browser_control':
                    return args.action ? `${args.action}` : status === 'constructing' ? 'preparing action...' : '';
                case 'browser_click':
                    return args.selector || args.text ? `click: ${args.selector || args.text}` : status === 'constructing' ? 'preparing click...' : 'click';
                case 'list_directory':
                    return args.path ? `path: ${args.path}` : status === 'constructing' ? 'preparing path...' : '';
                case 'run_command':
                    return args.command || (status === 'constructing' ? 'preparing command...' : '');
                case 'read_file':
                case 'write_file':
                    return args.path ? `file: ${args.path.split('/').pop()}` : status === 'constructing' ? 'preparing file operation...' : '';
                default:
...
(() => {
__webpack_require__.h = () => ("e19ff9e6414c2ced")
})();

}
);
//# sourceMappingURL=index.f71d5b92bbab0aa0.hot-update.js.map