...
const streamingToolCallsMap = new Map();
 ...
 */ const processEventAction = (0,jotai__WEBPACK_IMPORTED_MODULE_6__.atom)(null, (get, set, params)=>{
    ...
    switch(event.type){
        ...
        case 'agent_run_end':
            ...
            // Clean up streaming tool calls map when agent run ends
            streamingToolCallsMap.clear();
            break;
        case 'plan_start':
...
/**
 * Handle streaming tool call event - accumulate arguments and update UI immediately
 */ function handleStreamingToolCall(get, set, sessionId, event) {
    ...
    // Get or create streaming tool call entry
    let streamingCall = streamingToolCallsMap.get(toolCallId);
    if (!streamingCall) {
        streamingCall = {
            toolCallId,
            toolName,
        ...
        streamingToolCallsMap.set(toolCallId, streamingCall);
    }
    // Accumulate arguments
    streamingCall.arguments += argumentsDelta;
    streamingCall.messageId = messageId || streamingCall.messageId;
    // Parse accumulated arguments for UI display
    let parsedArgs = {};
    if (streamingCall.arguments.trim()) {
        try {
            // Use jsonrepair to safely parse potentially incomplete JSON
            const repairedJson = (0,jsonrepair__WEBPACK_IMPORTED_MODULE_8__.jsonrepair)(streamingCall.arguments);
            parsedArgs = JSON.parse(repairedJson);
        } catch (error) {
            // If parsing fails, keep empty object - UI will handle gracefully
            parsedArgs = {};
        }
    }
    // Create a streaming tool call object for UI
    ...
    // Update message with streaming tool call
    set(_atoms_message__WEBPACK_IMPORTED_MODULE_1__.messagesAtom, (prev)=>{
        const sessionMessages = prev[sessionId] || [];
        ...
        if (existingMessageIndex !== -1) {
            const existingMessage = sessionMessages[existingMessageIndex];
            // Update or add the streaming tool call
            ...
            if (existingToolCallIndex !== -1) {
                ...
                updatedToolCalls[existingToolCallIndex] = streamingToolCall;
            } else {
                // Add new streaming tool call
                updatedToolCalls.push(streamingToolCall);
            }
            const updatedMessage = {
                ...existingMessage,
        ...
        // If message doesn't exist yet, create a new streaming message
        const newMessage = {
            id: event.id || (0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])(),
            role: 'assistant',
            content: '',
    ...
    // For write_file tool specifically, show in workspace panel immediately
    if (toolName === 'write_file' && parsedArgs.path) {
        const content = parsedArgs.content || '';
        const fileName = parsedArgs.path ? parsedArgs.path.split('/').pop() || parsedArgs.path : 'Unknown file';
        set(_atoms_ui__WEBPACK_IMPORTED_MODULE_3__.activePanelContentAtom, {
            type: 'file',
            source: content,
    ...
    // If complete, clean up the streaming entry
    if (isComplete) {
        streamingToolCallsMap.delete(toolCallId);
        // Store final arguments for later use
 ...
 */ const ToolCalls = (param)=>{
    ...
    const getStatusIcon = (status, toolName, isStreaming)=>{
        // Show streaming indicator for streaming tool calls
        if (isStreaming) {
    ...
    const getToolDescription = (toolCall, isStreaming)=>{
        try {
            const args = JSON.parse(toolCall.function.arguments || '{}');
            // For streaming tool calls, show partial information
            if (isStreaming && toolCall.function.name === 'write_file') {
                if (args.path) {
                    const fileName = args.path.split('/').pop() || args.path;
                    const contentLength = args.content ? args.content.length : 0;
                    return contentLength > 0 ? `${fileName} (${contentLength} chars)` : fileName;
                }
    ...
    return /*#__PURE__*/ (0,react_jsx_dev_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxDEV)("div", {
        ...
        children: toolCalls.map((toolCall)=>{
            ...
            // Check if this is a streaming tool call (from parent message)
            const isStreaming = toolCall._isStreaming || false;
...
(() => {
__webpack_require__.h = () => ("003099a87ce89141")
})();

}
);
//# sourceMappingURL=index.60ac53b5b52cc691.hot-update.js.map