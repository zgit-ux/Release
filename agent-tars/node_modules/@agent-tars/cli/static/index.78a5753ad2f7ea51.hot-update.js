...
/**
 * Handle streaming tool call event - accumulate arguments and preview when possible
 */ function handleStreamingToolCall(get, set, sessionId, event) {
    ...
    // Skip if not write_file tool (as per requirements)
    if (toolName !== 'write_file') {
        return;
    }
    set(_atoms_tool__WEBPACK_IMPORTED_MODULE_2__.streamingToolCallsAtom, (prev)=>{
        const existing = prev[toolCallId];
        const accumulatedArguments = existing ? existing.accumulatedArguments + argumentsDelta : argumentsDelta;
        const streamingCall = {
            toolCallId,
            toolName,
            accumulatedArguments,
            messageId,
            startTime: (existing === null || existing === void 0 ? void 0 : existing.startTime) || Date.now()
        };
        // If complete, try to parse arguments and show preview
        if (isComplete && accumulatedArguments.trim()) {
            try {
                // Use jsonrepair for safe parsing of potentially incomplete JSON
                const repairedJson = (0,jsonrepair__WEBPACK_IMPORTED_MODULE_8__.jsonrepair)(accumulatedArguments);
                const parsedArgs = JSON.parse(repairedJson);
                // Store arguments for later use
                toolCallArgumentsMap.set(toolCallId, parsedArgs);
                // For write_file, show preview in workspace panel if we have content
                if (parsedArgs.content && parsedArgs.path) {
                    set(_atoms_ui__WEBPACK_IMPORTED_MODULE_3__.activePanelContentAtom, {
                        type: 'file',
                        source: parsedArgs.content,
                        title: `Writing: ${parsedArgs.path.split('/').pop() || parsedArgs.path}`,
                        timestamp: Date.now(),
                        toolCallId,
            ...
            // Remove from streaming state since it's complete
            const { [toolCallId]: removed, ...rest } = prev;
            return rest;
        }
        return {
            ...prev,
...
(() => {
__webpack_require__.h = () => ("482fdab3fb42a66a")
})();

}
);
//# sourceMappingURL=index.78a5753ad2f7ea51.hot-update.js.map