 ...
 */ const BrowserControlRenderer = (param)=>{
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        ...
        // Find the assistant message that contains the tool call to get its timestamp
        const assistantMessage = sessionMessages.find((msg)=>{
            var _msg_toolCalls;
            return (_msg_toolCalls = msg.toolCalls) === null || _msg_toolCalls === void 0 ? void 0 : _msg_toolCalls.some((tc)=>tc.id === toolCallId);
        });
        if (!assistantMessage) {
            // Fallback logic, especially for replay mode where message might not be found immediately.
            // Simply find the latest environment message available.
            const envMessagesWithImages = sessionMessages.filter((msg)=>msg.role === 'environment' && Array.isArray(msg.content) && msg.content.some((c)=>c.type === 'image_url'));
            if (envMessagesWithImages.length > 0) {
                const latestEnvMessage = envMessagesWithImages[envMessagesWithImages.length - 1];
                const imgContent = latestEnvMessage.content.find((c)=>c.type === 'image_url');
        ...
        // Primary logic: Find the latest screenshot *before* the assistant message
        const precedingEnvMessages = sessionMessages.filter((msg)=>msg.role === 'environment' && msg.timestamp < assistantMessage.timestamp && Array.isArray(msg.content) && msg.content.some((c)=>c.type === 'image_url'));
        if (precedingEnvMessages.length > 0) {
            // Sort by timestamp descending and take the first one
            const latestEnvMessage = precedingEnvMessages.sort((a, b)=>b.timestamp - a.timestamp)[0];
            const imgContent = latestEnvMessage.content.find((c)=>c.type === 'image_url');
            if (imgContent && 'image_url' in imgContent && imgContent.image_url.url) {
                setRelatedImage(imgContent.image_url.url);
            }
...
(() => {
__webpack_require__.h = () => ("eb071a631c12dda7")
})();

}
);
//# sourceMappingURL=index.f73ff336ea3fca0c.hot-update.js.map