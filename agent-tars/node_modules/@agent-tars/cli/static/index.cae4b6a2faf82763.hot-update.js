 ...
 */ const BrowserControlRenderer = (param)=>{
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        ...
        // Find the message containing the tool call to get its timestamp
        const toolCallMessage = sessionMessages.find((msg)=>{
            var _msg_toolCalls;
        ...
        const toolCallTimestamp = toolCallMessage.timestamp;
        // Find all preceding environment messages with an image
        const precedingEnvImages = sessionMessages.filter((msg)=>msg.role === 'environment' && msg.timestamp < toolCallTimestamp && Array.isArray(msg.content) && msg.content.some((c)=>{
                var _c_image_url;
                return typeof c === 'object' && c !== null && c.type === 'image_url' && ((_c_image_url = c.image_url) === null || _c_image_url === void 0 ? void 0 : _c_image_url.url);
            }));
        if (precedingEnvImages.length > 0) {
            // Get the one with the latest timestamp (closest to the tool call)
            const latestEnvMessage = precedingEnvImages.reduce((latest, current)=>current.timestamp > latest.timestamp ? current : latest);
            const imgContent = latestEnvMessage.content.find((c)=>{
                var _c_image_url;
                return c.type === 'image_url' && ((_c_image_url = c.image_url) === null || _c_image_url === void 0 ? void 0 : _c_image_url.url);
            });
            if (imgContent) {
            ...
            // If no preceding image is found, clear any existing one
            setRelatedImage(null);
        }
    }, [
        activeSessionId,
...
(() => {
__webpack_require__.h = () => ("52c96c7b05b6a3bd")
})();

}
);
//# sourceMappingURL=index.cae4b6a2faf82763.hot-update.js.map