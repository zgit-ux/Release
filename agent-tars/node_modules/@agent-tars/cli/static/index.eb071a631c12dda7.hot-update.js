 ...
 */ function handleEnvironmentInput(set, sessionId, event) {
    ...
    // Also add screenshots to tool results so they appear in the workspace
    if (Array.isArray(event.content)) {
        const imageContent = event.content.find((part)=>part.type === 'image_url');
        if (imageContent) {
            const result = {
                id: (0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])(),
                toolCallId: event.id,
                name: event.description || 'Screenshot',
                content: event.content,
                timestamp: event.timestamp,
                type: 'image'
            };
...
(() => {
__webpack_require__.h = () => ("3ff4bd8ba39f8cae")
})();

}
);
//# sourceMappingURL=index.eb071a631c12dda7.hot-update.js.map