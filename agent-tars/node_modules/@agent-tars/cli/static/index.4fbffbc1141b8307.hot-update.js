 ...
 * - Assistant messages can be clicked to jump to related environment input
 */ const Message = (param)=>{
    let { message, isIntermediate = false, isInGroup = false, shouldDisplayTimestamp = true, onScrollToMessage } = param;
    _s();
    ...
    // Find the most recent environment input before this assistant message
    const findNearestEnvironmentInput = react__WEBPACK_IMPORTED_MODULE_1___default().useCallback(()=>{
        if (!activeSessionId || !isAssistantMessage) return null;
        const sessionMessages = allMessages[activeSessionId] || [];
        const currentMessageIndex = sessionMessages.findIndex((msg)=>msg.id === message.id);
        if (currentMessageIndex === -1) return null;
        // Search backwards from current message
        for(let i = currentMessageIndex - 1; i >= 0; i--){
            const msg = sessionMessages[i];
            if (msg.role === 'environment' && Array.isArray(msg.content)) {
                const hasImage = msg.content.some((part)=>part && typeof part === 'object' && 'type' in part && part.type === 'image_url');
                if (hasImage) {
                    return msg.id;
                }
            }
    ...
    // Handle assistant message click
    const handleAssistantClick = react__WEBPACK_IMPORTED_MODULE_1___default().useCallback(()=>{
        if (!isAssistantMessage || !onScrollToMessage) return;
        const nearestEnvInputId = findNearestEnvironmentInput();
        if (nearestEnvInputId) {
            onScrollToMessage(nearestEnvInputId);
        }
    }, [
        isAssistantMessage,
        onScrollToMessage,
        findNearestEnvironmentInput
    ]);
    // Handle tool call click - show in panel
    const handleToolCallClick = (0,react__WEBPACK_IMPORTED_MODULE_1__.useCallback)((toolCall)=>{
    ...
    // Check if this assistant message has a related environment input
    const hasNearestEnvInput = react__WEBPACK_IMPORTED_MODULE_1___default().useMemo(()=>{
        return isAssistantMessage && findNearestEnvironmentInput() !== null;
    }, [
        isAssistantMessage,
        findNearestEnvironmentInput
    ]);
    const messageElement = /*#__PURE__*/ (0,react_jsx_dev_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxDEV)(framer_motion__WEBPACK_IMPORTED_MODULE_18__.motion.div, {
        initial: "initial",
        animate: "animate",
        ...
        children: [
            /*#__PURE__*/ (0,react_jsx_dev_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxDEV)("div", {
                className: `message-bubble ${getMessageBubbleClasses()} ${isIntermediate ? 'message-bubble-intermediate' : ''} ${hasNearestEnvInput ? 'cursor-pointer hover:bg-opacity-90 transition-all duration-200' : ''}`,
                onClick: hasNearestEnvInput ? handleAssistantClick : undefined,
                title: hasNearestEnvInput ? 'Click to jump to related screenshot' : undefined,
...
_s(Message, "ffGGh3/JQ5oxqFrp7hXn5LymgJ4=", false, function() {
    return [
...
(() => {
__webpack_require__.h = () => ("1267d85acaf6c29e")
})();

}
);
//# sourceMappingURL=index.4fbffbc1141b8307.hot-update.js.map