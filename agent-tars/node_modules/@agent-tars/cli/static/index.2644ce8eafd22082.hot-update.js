 ...
 */ const BrowserControlRenderer = (param)=>{
    ...
    const { thought, step, action, status, toolCallId, timestamp } = part;
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        if (!activeSessionId || !timestamp) return;
        console.log('[GUI] messages', messages);
        const sessionMessages = messages[activeSessionId] || [];
        // Filter for environment messages that occurred at or before the current tool result's timestamp
        const precedingEnvMessages = sessionMessages.filter((msg)=>msg.role === 'environment' && msg.timestamp <= timestamp);
        console.log('[GUI] precedingEnvMessages', precedingEnvMessages);
        // If there are preceding environment messages, find the latest one
        if (precedingEnvMessages.length > 0) {
            const latestEnvMessage = precedingEnvMessages.reduce((latest, current)=>latest.timestamp > current.timestamp ? latest : current);
            if (Array.isArray(latestEnvMessage.content)) {
                const imgContent = latestEnvMessage.content.find((c)=>{
                    var _c_image_url;
                    return typeof c === 'object' && c && 'type' in c && c.type === 'image_url' && 'image_url' in c && ((_c_image_url = c.image_url) === null || _c_image_url === void 0 ? void 0 : _c_image_url.url);
                });
                console.log('imgContent', imgContent);
                if (imgContent && 'image_url' in imgContent) {
        ...
        console.warn(`[BrowserControlRenderer] Could not find a preceding screenshot for timestamp: ${timestamp}.`);
...
(() => {
__webpack_require__.h = () => ("884945097003a2f8")
})();

}
);
//# sourceMappingURL=index.2644ce8eafd22082.hot-update.js.map