...
/* ESM import */var jsonrepair__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__("../node_modules/.pnpm/jsonrepair@3.12.0/node_modules/jsonrepair/lib/esm/regular/jsonrepair.js");
...
const streamingToolCallArgsMap = new Map();
// Throttle state for streaming tool calls
const throttleStateMap = new Map();
/**
 ...
 * Throttled to maximum 2 updates per second for better performance
 */ function handleStreamingToolCall(get, set, sessionId, event) {
    ...
    // Always accumulate arguments immediately for data accuracy
    ...
    // Always try to parse arguments for data consistency
    let parsedArgs = {};
    try {
        if (newArgs.trim()) {
            const repairedJson = (0,jsonrepair__WEBPACK_IMPORTED_MODULE_8__.jsonrepair)(newArgs);
            parsedArgs = JSON.parse(repairedJson);
        }
    } catch (error) {
        parsedArgs = {};
    }
    // Always store current arguments for tool result processing
    toolCallArgumentsMap.set(toolCallId, parsedArgs);
    // Throttle UI updates
    if (isComplete) {
        updateMessageAndPanel(get, set, sessionId, {
            toolCallId,
            toolName,
            messageId,
            newArgs,
    ...
    // Get or initialize throttle state for this tool call
    const currentTime = Date.now();
    let throttleState = throttleStateMap.get(toolCallId);
    if (!throttleState) {
        throttleState = {
            timer: null,
            callCount: 0,
        ...
        throttleStateMap.set(toolCallId, throttleState);
    }
    // Check if we can perform immediate update (within 1s window, less than 2 calls)
    const timeWindow = 1000; // 1 second
    const maxCallsPerWindow = 2;
    if (currentTime - throttleState.lastCallTime > timeWindow) {
        // Reset window
        throttleState.callCount = 0;
        throttleState.lastCallTime = currentTime;
    }
    if (throttleState.callCount < maxCallsPerWindow) {
        // Can perform immediate update
        throttleState.callCount++;
        throttleState.pendingUpdate = false;
        updateMessageAndPanel(get, set, sessionId, {
            toolCallId,
            toolName,
            messageId,
        ...
        // Need to throttle - schedule delayed update if not already pending
        if (!throttleState.pendingUpdate) {
            throttleState.pendingUpdate = true;
            if (throttleState.timer) {
                clearTimeout(throttleState.timer);
            }
            const delay = timeWindow - (currentTime - throttleState.lastCallTime);
            throttleState.timer = setTimeout(()=>{
                throttleState.callCount = 1;
                throttleState.lastCallTime = Date.now();
                throttleState.pendingUpdate = false;
                throttleState.timer = null;
                // Perform update with latest data
                const latestArgs = streamingToolCallArgsMap.get(toolCallId) || '';
                let latestParsedArgs = {};
                try {
                    if (latestArgs.trim()) {
                        const repairedJson = (0,jsonrepair__WEBPACK_IMPORTED_MODULE_8__.jsonrepair)(latestArgs);
                        latestParsedArgs = JSON.parse(repairedJson);
                    }
                } catch (error) {
                    latestParsedArgs = {};
                }
...
/**
 * Perform the actual UI update for streaming tool calls
 */ function updateMessageAndPanel(get, set, sessionId, params) {
    const { toolCallId, toolName, messageId, newArgs, parsedArgs, isComplete, timestamp } = params;
    ...
    set(_atoms_message__WEBPACK_IMPORTED_MODULE_1__.messagesAtom, (prev)=>{
        ...
        if (existingMessageIndex !== -1) {
            ...
            if (toolCallIndex !== -1) {
                // Update existing tool call - use JSON string for message storage
                ...
                // Add new tool call - use JSON string for message storage
        ...
        // Create new message if not found - use JSON string for message storage
        const newMessage = {
            id: (0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])(),
            role: 'assistant',
            content: '',
    ...
    // For write_file tool, update workspace panel with parsed args
...
(() => {
__webpack_require__.h = () => ("7a2ca578abd13b5a")
})();

}
);
//# sourceMappingURL=index.62b1cf7387a18c96.hot-update.js.map