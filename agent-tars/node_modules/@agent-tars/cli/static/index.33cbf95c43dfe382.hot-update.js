...
// 存储流式工具调用的累加参数
const streamingToolCallArgumentsMap = new Map();
/**
...
/**
 * Handle streaming tool call event - accumulate arguments and update workspace
 */ function handleStreamingToolCall(get, set, sessionId, event) {
    const { toolCallId, toolName, arguments: argumentsDelta, isComplete } = event;
    // Accumulate arguments
    const currentArgs = streamingToolCallArgumentsMap.get(toolCallId) || '';
    const newArgs = currentArgs + argumentsDelta;
    streamingToolCallArgumentsMap.set(toolCallId, newArgs);
    // Try to parse accumulated arguments safely
    let parsedArgs = {};
    if (newArgs.trim()) {
        try {
            parsedArgs = JSON.parse((0,jsonrepair__WEBPACK_IMPORTED_MODULE_8__.jsonrepair)(newArgs));
        } catch (error) {
            // If parsing fails, keep the raw string for debugging
            console.debug(`Failed to parse streaming tool call arguments for ${toolCallId}:`, error);
            parsedArgs = {
                _raw: newArgs
            };
        }
    ...
    // Create a preview ToolResult for workspace display
    const previewResult = {
        id: `preview-${toolCallId}`,
        toolCallId,
        name: toolName,
        content: isComplete ? 'Tool call completed' : 'Tool call in progress...',
        timestamp: event.timestamp,
        type: (0,_common_utils_formatters__WEBPACK_IMPORTED_MODULE_0__.determineToolType)(toolName, parsedArgs),
        arguments: parsedArgs,
        // Mark as streaming/preview
        _isStreaming: true,
        _isComplete: isComplete
    };
    // Update workspace panel with preview for write_file and other relevant tools
    if (shouldShowStreamingPreview(toolName)) {
        set(_atoms_ui__WEBPACK_IMPORTED_MODULE_3__.activePanelContentAtom, {
            type: previewResult.type,
            source: previewResult.content,
            title: `${toolName} (${isComplete ? 'Completing' : 'In Progress'})`,
            timestamp: event.timestamp,
            toolCallId,
            arguments: parsedArgs,
    ...
    if (isComplete) {
        streamingToolCallArgumentsMap.delete(toolCallId);
    }
}
/**
 * Determine if a tool should show streaming preview in workspace
 */ function shouldShowStreamingPreview(toolName) {
    // Focus on write_file and other file-related tools that benefit from streaming preview
 ...
 * - Supports streaming tool calls with live progress indication
 */ const ToolCalls = (param)=>{
    ...
    const getToolCallStatus = (toolCall)=>{
        ...
        if (result._isStreaming && !result._isComplete) {
            return 'streaming'; // Tool is streaming
        }
        if (result.error) {
    ...
    // 生成工具描述文本 - 增强描述信息的可读性
    const getToolDescription = (toolCall, status)=>{
        try {
            const args = JSON.parse(toolCall.function.arguments || '{}');
            // For streaming tools, show progress indication
            if (status === 'streaming') {
                switch(toolCall.function.name){
                    case 'write_file':
                        return args.path ? `writing to ${args.path.split('/').pop()}...` : 'writing file...';
                    case 'edit_file':
                        return args.path ? `editing ${args.path.split('/').pop()}...` : 'editing file...';
                    default:
                        return 'in progress...';
                }
            }
...
(() => {
__webpack_require__.h = () => ("fad2681ffdcd6d32")
})();

}
);
//# sourceMappingURL=index.33cbf95c43dfe382.hot-update.js.map