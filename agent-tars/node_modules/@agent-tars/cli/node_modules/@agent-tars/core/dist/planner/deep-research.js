...
class DeepResearchGenerator {
    async generateReport(llmClient, resolvedModel, eventStream, options, abortSignal) {
        try {
            ...
            const messageId = `research-report-${(0, core_namespaceObject.uuidv4)()}`;
    ...
    extractRelevantData(eventStream) {
        ...
        const userMessages = events.filter((e)=>'user_message' === e.type);
        const toolResults = events.filter((e)=>'tool_result' === e.type);
        const assistantMessages = events.filter((e)=>'assistant_message' === e.type);
        const environmentInputs = events.filter((e)=>'environment_input' === e.type);
        ...
        const browserContent = toolResults.filter((result)=>{
            var _result_name;
            return null == (_result_name = result.name) ? void 0 : _result_name.includes('browser_get_markdown');
        }).map((result)=>result.content).filter(Boolean);
        const searchResults = toolResults.filter((result)=>{
            var _result_name;
            return null == (_result_name = result.name) ? void 0 : _result_name.includes('search');
        }).map((result)=>result.content).filter(Boolean);
        ...
        environmentInputs.forEach((input)=>{
            if (Array.isArray(input.content)) input.content.forEach((contentPart)=>{
                ...
                if ('image_url' === contentPart.type && (null == (_contentPart_image_url = contentPart.image_url) ? void 0 : _contentPart_image_url.url)) environmentImages.push(contentPart);
    ...
    async generateReportStructure(llmClient, resolvedModel, relevantData, options, abortSignal) {
        try {
            ...
            const structureContent = (null == (_response_choices_ = response.choices[0]) ? void 0 : null == (_response_choices__message = _response_choices_.message) ? void 0 : _response_choices__message.content) || '{}';
            const reportStructure = JSON.parse(structureContent);
            this.logger.info(`Generated report structure with ${(null == (_reportStructure_sections = reportStructure.sections) ? void 0 : _reportStructure_sections.length) || 0} sections`);
...
exports.DeepResearchGenerator = __webpack_exports__.DeepResearchGenerator;
