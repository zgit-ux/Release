...
const index_js_namespaceObject = require("./browser/index.js");
const plan_manager_js_namespaceObject = require("./planner/plan-manager.js");
const external_search_index_js_namespaceObject = require("./search/index.js");
const server_cjs_namespaceObject = require("@agent-infra/mcp-server-browser/dist/server.cjs");
const mcp_server_filesystem_namespaceObject = require("@agent-infra/mcp-server-filesystem");
const mcp_server_commands_namespaceObject = require("@agent-infra/mcp-server-commands");
...
class AgentTARS extends core_namespaceObject.MCPAgent {
    async initialize() {
        ...
        try {
            ...
            this.browserToolsManager = new index_js_namespaceObject.BrowserToolsManager(this.logger, control);
    ...
    async initializeSearchTools() {
        try {
            ...
            this.searchToolProvider = new external_search_index_js_namespaceObject.SearchToolProvider(this.logger, {
...
\u{1F4E6} ${category} (${toolNames.length}):`);
    ...
    async initializeGUIAgent() {
        try {
            ...
            this.browserGUIAgent = new index_js_namespaceObject.BrowserGUIAgent({
                logger: this.logger,
    ...
    async initializeInMemoryMCPForBuiltInMCPServers() {
        try {
            ...
            const mcpModules = {
                browser: server_cjs_namespaceObject,
                filesystem: mcp_server_filesystem_namespaceObject,
                commands: mcp_server_commands_namespaceObject
            };
            this.mcpServers = {
            ...
            await Promise.all(Object.entries(this.mcpServers).filter(([_, server])=>null !== server).map(async ([name, server])=>{
                const [clientTransport, serverTransport] = core_namespaceObject.InMemoryTransport.createLinkedPair();
                const client = new core_namespaceObject.Client({
                    name: `${name}-client`,
                    version: '1.0'
                }, {
                    capabilities: {
    ...
    constructor(options){
        ...
        let plannerPrompt = '';
        if (null == plannerOptions ? void 0 : plannerOptions.enabled) plannerPrompt = `${plan_manager_js_namespaceObject.DEFAULT_PLANNING_PROMPT} \n\n ${plannerOptions.planningPrompt ?? ''}`;
        const browserRules = (0, external_prompt_js_namespaceObject.generateBrowserRulesPrompt)(null == (_tarsOptions_browser = tarsOptions.browser) ? void 0 : _tarsOptions_browser.control);
        const systemPrompt = `${external_prompt_js_namespaceObject.DEFAULT_SYSTEM_PROMPT}
${plannerPrompt ? `\n${plannerPrompt}` : ''}
${browserRules}
...
Current Working Directory: ${workingDirectory}
    ...
    `;
        ...
        this.browserManager = index_js_namespaceObject.BrowserManager.getInstance(this.logger);
        if (null == plannerOptions ? void 0 : plannerOptions.enabled) this.planManager = new plan_manager_js_namespaceObject.PlanManager(this.logger, this.eventStream, this, plannerOptions);
...
exports.AgentTARS = __webpack_exports__.AgentTARS;
...
//# sourceMappingURL=agent-tars.js.map