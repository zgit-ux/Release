...
import { ImageCompressor, formatBytes } from "../shared/utils.mjs";
    ...
    async onEachAgentLoopStart(eventStream, isReplaySnapshot = false) {
        ...
        try {
            ...
            console.log('Screenshot info:', {
                ...
                size: `${sizeInKB} KB`,
                time: `${screenshotTime} ms`,
                compression: `${originalSize / 1024 > 1024 ? (originalSize / 1024 / 1024).toFixed(2) + ' MB' : (originalSize / 1024).toFixed(2) + ' KB'} \u{2192} ${formatBytes(compressedSize)} (${compressionPercentage}% reduction)`
            });
            const event = eventStream.createEvent('environment_input', {
                content: [
                    {
...
`,
            parameters: z.object({
                thought: z.string().describe('Your observation and small plan in one sentence, DO NOT include " characters to avoid failure to render in JSON'),
                step: z.string().describe('Finally summarize the next action (with its target element) in one sentence'),
...
export { BrowserGUIAgent };

//# sourceMappingURL=browser-gui-agent.mjs.map