...
class GUIAgent {
    async capturePageContentAsEnvironmentInfo() {
        ...
        try {
            ...
            if (markdown && markdown.trim()) {
                const event = this.eventStream.createEvent(mcp_agent_namespaceObject.EventType.ENVIRONMENT_INPUT, {
                    content: markdown,
    ...
    async onEachAgentLoopStart(eventStream, isReplaySnapshot = false) {
        ...
        if (isReplaySnapshot) {
            const event = eventStream.createEvent(mcp_agent_namespaceObject.EventType.ENVIRONMENT_INPUT, {
                content: [
                    {
                        type: 'image_url',
                        image_url: {
    ...
    constructor(options){
        ...
        this.guiAgentTool = new mcp_agent_namespaceObject.Tool({
...
`,
            parameters: mcp_agent_namespaceObject.z.object({
                thought: mcp_agent_namespaceObject.z.string().describe('Your observation and small plan in one sentence, DO NOT include " characters to avoid failure to render in JSON'),
                step: mcp_agent_namespaceObject.z.string().describe('Finally summarize the next action (with its target element) in one sentence'),
                action: mcp_agent_namespaceObject.z.string().describe('Some action in action space like click or press')
            }),
