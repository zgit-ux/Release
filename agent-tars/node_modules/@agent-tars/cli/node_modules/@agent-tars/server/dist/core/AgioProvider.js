...
const agio_namespaceObject = require("@multimodal/agio");
const core_namespaceObject = require("@agent-tars/core");
const external_AgioBatchProcessor_js_namespaceObject = require("./AgioBatchProcessor.js");
...
class AgioProvider {
    ...
    async sendAgentInitialized() {
        ...
        const event = agio_namespaceObject.AgioEvent.createEvent('agent_initialized', this.sessionId, {
            config: {
    ...
    async handleRunStart(event) {
        ...
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('agent_run_start', this.sessionId, {
            runId: this.runId,
    ...
    async handleRunEnd(event) {
        ...
        const successful = event.status !== core_namespaceObject.AgentStatus.ERROR;
        const isError = event.status === core_namespaceObject.AgentStatus.ERROR;
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('agent_run_end', this.sessionId, {
            runId: this.runId,
            executionTimeMs,
    ...
    async handleFirstToken(event) {
        if (!this.firstTokenTime && this.runStartTime && event.content) {
            ...
            const agioEvent = agio_namespaceObject.AgioEvent.createEvent('agent_ttft', this.sessionId, {
                runId: this.runId,
    ...
    async handleToolCall(event) {
        ...
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('tool_call', this.sessionId, {
            runId: this.runId,
            toolName: event.name,
    ...
    async handleToolResult(event) {
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('tool_result', this.sessionId, {
            runId: this.runId,
            toolName: event.name,
            toolCallId: event.toolCallId,
    ...
    async handleLoopStart() {
        ...
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('agent_loop_start', this.sessionId, {
            runId: this.runId,
    ...
    async handleLoopEnd(event) {
        ...
        const agioEvent = agio_namespaceObject.AgioEvent.createEvent('agent_loop_end', this.sessionId, {
            runId: this.runId,
            iteration: this.currentIteration,
            durationMs
        });
    ...
    constructor(providerUrl, appConfig, sessionId, agent){
        ...
        this.batchProcessor = new external_AgioBatchProcessor_js_namespaceObject.AgioBatchProcessor({
...
//# sourceMappingURL=AgioProvider.js.map